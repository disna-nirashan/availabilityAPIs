...
//    @Query("SELECT r.roomTypeKey FROM RoomType r WHERE (r.adultCount+r.infantCount)>=:totalFax AND r.adultCount>=:adultCount")
    ...
    @Query("select r.roomTypeKey from RoomType r where ((COALESCE(r.adultCount,0) + COALESCE(r.infantCount,0))>= COALESCE(:totalFax,0) AND COALESCE(r.adultCount ,0)>= COALESCE(:adultCount ,0)) OR ( COALESCE(r.totalFax ,0)>=COALESCE(:totalFax ,0) AND r.roomTypeID='Family') ")

    List<Integer> findRoomTypes(@Param("adultCount") int adultCount,@Param("totalFax") int totalFax);
}
